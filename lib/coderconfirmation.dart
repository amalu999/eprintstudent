import 'package:eprintapp_deco/homepage.dart';
...
class ConfirmcOrder extends StatefulWidget {
  ...
  final int copies;
  final rate;
  const ConfirmcOrder({Key key, this.subject, this.nameOfBook, this.author, this.beg, this.end, this.copies,this.rate}) : super(key: key);
...
class _ConfirmcOrderState extends State<ConfirmcOrder> {
  ...
  void corderplacing() async {
    ...
    var data = {
      ...
      "beg": beg,
      "end": end,
      "amt":((int.parse(widget.end.toString())-int.parse(widget.beg.toString())+1)*int.parse(widget.copies.toString())*double.parse(widget.rate.toString())).toString()
      ,
    };
              ...
              Padding(
                ...
                child: RichText(
                  text: TextSpan(
                      ...
                      style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: Colors.black),
                      children: [
                        TextSpan(text: widget.rate.toString(),style: TextStyle(
                            fontWeight: FontWeight.normal,color: Colors.blueGrey
                        ))
                      ]
                  ),
              ...
              Padding(
                ...
                child: RichText(
                  text: TextSpan(
                      ...
                      style: TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: Colors.black),
                      children: [
                        TextSpan(text: ((int.parse(widget.end.toString())-int.parse(widget.beg.toString())+1)*int.parse(widget.copies.toString())*double.parse(widget.rate.toString())).toString(),style: TextStyle(
                            fontWeight: FontWeight.normal,color: Colors.blueGrey
                        ))
                      ]
                  ),
